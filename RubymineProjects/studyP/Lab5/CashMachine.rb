...
Please choose existing command."
      end
    end
  ...
  def deposit
    ...
    if float_arg_error?(input)
      puts "Deposit value is
incorrect! Try to insert number."
      return 'error'
    end
    dep = input.to_f
    if dep < 0
  ...
  def withdraw
    ...
    input = gets
    if float_arg_error?(input)
      puts "Withdraw value is incorrect!
Try to insert number."
      return 'error'
    end
    wd = input.to_f
    if wd <0 || wd > @balance
      puts "Withdraw value must be more
than 0 and not greater than balance (#{@balance})"
    else
      @balance -= wd
    end
    @balance
